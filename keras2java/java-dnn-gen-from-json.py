...
##  DNN ==> JAVA
def java_convert(layers):

  consts=''
  prog=''

  ##  input, we assume that channel is in behind
  input_shape=layers['0']['inp_sp']
  print ('####', input_shape)
  ...
  for l in range(0, len(layers)):
    ...
    #_inp=layer.input
    #_out=layer.output
    _inp_sp = layer['inp_sp'] #= _inp.shape.as_list() # json
    _out_sp = layer['out_sp'] #= _out.shape.as_list() # json
    #is_conv=is_conv_layer(layer)
    is_conv = layer['is_conv'] #= is_conv # json
    if is_conv:
      is_padding = layer['is_padding']  # json
    #is_dense=is_dense_layer(layer)
    is_dense = layer['is_dense'] #= is_dense # json
    #is_activation=is_activation_layer(layer)
    is_activation = layer['is_activation'] #= is_activation # json
    #is_maxpooling=is_maxpooling_layer(layer)
    is_maxpooling = layer['is_maxpooling'] #= is_maxpooling # json
    #is_flatten=is_flatten_layer(layer)
    is_flatten = layer['is_flatten'] #= is_flatten # json
    ...
    is_relu = layer['is_relu'] #= is_relu # json
    ...
    if is_dense or is_conv:
      ...
      #print (l, weights)
      #sp=weights.shape
      sp = layer['w_sp'] #= sp # json
      bsp = layer['b_sp'] #= biases.shape # json
      if is_conv:
    ...
    if is_conv:
      ...
      s=_inp_sp[1] * _inp_sp[2] * _inp_sp[3]
      prog+='    double[] layer{1}=new double[{0}];\n'.format(s, l)
      prog+='    for(int i=0; i<{0}; i++)\n'.format(s)
      prog+='    {\n'
      prog+='      int d0=i/{0};\n'.format(_inp_sp[2] * _inp_sp[3])
      prog+='      int d1=(i%{0})/{1};\n'.format(_inp_sp[2] * _inp_sp[3], _inp_sp[3])
      ...
      pool_size = layer['pool_size'] #= pool_size # json
      prog+='    double[][][] layer{3}=new double[{0}][{1}][{2}];\n'.format(_out_sp[1], _out_sp[2], _out_sp[3], l)
      prog+='    for(int i=0; i<{0}; i++)\n'.format(_out_sp[1])
      prog+='      for(int j=0; j<{0}; j++)\n'.format(_out_sp[2])
      prog+='        for(int k=0; k<{0}; k++)\n'.format(_out_sp[3])
...
def main():
  ...
  parser.add_argument('model', action='store', nargs='+', help='The input neural network model (.json)')


  args=parser.parse_args()
  with open(args.model[0]) as json_file:
